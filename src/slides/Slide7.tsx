...
import { ChevronLeft, ChevronRight, Download, FileText, Shield, CheckCircle } from 'lucide-react';

type AnySlide = Record<string, any>;

const SlideShow: React.FC = () => {
  const [currentSlide, setCurrentSlide] = useState<number>(0);
  const slides = [/* … your existing slides for Part 7 … */] as const;
  ...
  const downloadAsHTML = () => {
    ...
    a.href = url; a.download = 'Part7_Validation_Ethics.html';
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    URL.revokeObjectURL(url);
  };

  const generateFullHTML = (): string => `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Part 7: Validation & Ethics</title></head>
<body>${(slides as unknown as AnySlide[]).map((s) => `<div><h1>${s.title || 'Content'}</h1></div>`).join('')}</body></html>`;

  const renderSlide = (slide: AnySlide) => {
    // keep your render switch; add typing to all .map callbacks and entries casts
    return <div />; // placeholder — replace with your existing render code
  };
    ...
    <div className="w-full h-screen flex flex-col bg-gray-100">
      ...
      <div className="bg-gray-800 text-white p-4 flex items-center justify-between no-print">
        <button onClick={prevSlide} disabled={currentSlide === 0}
          className={`flex items-center px-6 py-3 rounded-lg font-semibold transition-all ${currentSlide === 0 ? 'bg-gray-600 cursor-not-allowed opacity-50' : 'bg-rose-600 hover:bg-rose-700'}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={nextSlide} disabled={currentSlide === slides.length - 1}
            className={`flex items-center px-6 py-3 rounded-lg font-semibold transition-all ${currentSlide === slides.length - 1 ? 'bg-gray-600 cursor-not-allowed opacity-50' : 'bg-rose-600 hover:bg-rose-700'}`}>
